...
Integration Points:
  ...
  - iface-llm-provider :: LLM Provider | kind: interface | summary: "The hosted LLM (OpenAI/Anthropic per backend/.env.example) the Planner, Calculation Agent, and Synthesizer all call. One shared external dependency, not three." | spec: [system-architecture.md Technology Baseline](../../system-architecture.md#technology-baseline) | shape: "Prompt/response per agent; exact contracts are TRD (50a) work" | version: "unversioned — third-party service" | compatibility: "breaking — outside this module's control"
